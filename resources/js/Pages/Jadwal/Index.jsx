...
export default function Index({
    ...
    // Pencarian & filter dijalankan di server karena daftar sudah dipaginasi.
    const [search, setSearch] = useState(filters.cari || '');
    const [filterType, setFilterType] = useState(filters.jenis || 'Semua');
    ...
    const selectAllStudents = () => {
        ...
        setSelectedStudents(
            Array.from(new Set([...selectedStudents, ...visibleNims])),
        );
    };
    ...
    const deselectAllStudents = () => {
        const visibleNims = filteredStudents.map((s) => s.nim);
        setSelectedStudents(
            selectedStudents.filter((id) => !visibleNims.includes(id)),
        );
    };
    ...
    const uniqueClasses = Array.from(
        new Set(mahasiswas.map((m) => m.kelas).filter(Boolean)),
    ).sort();
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            router.get(
                ...
                {
                    cari: search,
                    jenis: filterType === 'Semua' ? '' : filterType,
    ...
    const isTheoryOnly = selectedCourse
        ? selectedCourse.teori && !selectedCourse.praktek
        : false;

    const dosenOptions = dosens.map((d) => {
        const ampuMK = Array.isArray(d.ampu_mata_kuliah)
            ? d.ampu_mata_kuliah
            : [];
        const ampuKelas = Array.isArray(d.ampu_kelas) ? d.ampu_kelas : [];

        const isAmpuCourse = ampuMK.includes(form.data.kode_mk);
        const isAmpuClass = ampuKelas
            .map((k) => k.trim().toUpperCase())
            .includes((form.data.kelas || '').trim().toUpperCase());
    ...
    // Cacahan dihitung di server atas seluruh data, bukan hanya halaman ini.
    const totalCount = stats?.total ?? 0;
    const scheduledCount = stats?.terjadwal ?? 0;
    const activeCount = stats?.berlangsung ?? 0;
    const completedCount = stats?.selesai ?? 0;
    const cancelledCount = stats?.dibatalkan ?? 0;
            ...
            >
                ...
                <div className="sibau-card sibau-stat-card">
                    ...
                    <div className="sibau-stat-card-body">
                        <div
                            className="sibau-stat-icon-wrapper"
            ...
            {importModalOpen && (
                <div className="sibau-modal-overlay">
                    <div className="sibau-modal">
                        <div className="sibau-modal-header">
                            <h3 className="sibau-modal-title">
                                Import Jadwal Ujian dari Excel
                            </h3>
                            <button
                            ...
                            <div className="sibau-modal-body">
                                ...
                                >
                                    ...
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <input
                                            ...
                                            className="sibau-input"
                                            value={
                                                importForm.data.semester_aktif
                                            }
                                            onChange={(e) =>
                                                importForm.setData(
                                                    'semester_aktif',
                                ...
                                <div className="sibau-form-group">
                                    ...
                                    <input
                                        type="text"
                                        className="sibau-input"
                                        value={importForm.data.tahun_akademik}
                                        onChange={(e) =>
                                            importForm.setData(
                                                'tahun_akademik',
                                                e.target.value,
                                ...
                                >
                                    ...
                                    <br />
                                    Format baris ke-6 tabel Excel harus
                                    berurutan:
                                    <br />
                                    <strong>
                                        (Hari/Tanggal, Jam, Ruang 1, Ruang 2,
                                        Mata Kuliah, SKS, Kls, Jml Mhs, Dosen)
                                    </strong>
                                </div>
                            </div>
                            <div className="sibau-modal-footer">
