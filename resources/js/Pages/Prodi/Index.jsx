...
export default function Index({ prodis }) {
    ...
    const handleAddClass = (e) => {
        ...
        classForm.setData('daftar_kelas', [
            ...classForm.data.daftar_kelas,
            cleaned,
        ]);
        setNewClassName('');
    };
    ...
    const handleDeleteClass = (clsName) => {
        classForm.setData(
            'daftar_kelas',
            classForm.data.daftar_kelas.filter((c) => c !== clsName),
        );
    };
    ...
    const submit = (e) => {
        ...
        if (editingProdi) {
            form.put(
                route('admin.prodi.update', {
                    kode_prodi: editingProdi.kode_prodi,
                }),
                {
    ...
    const deleteProdi = (kode_prodi) => {
        if (confirm('Apakah Anda yakin ingin menghapus Program Studi ini?')) {
            form.delete(
                route('admin.prodi.delete', { kode_prodi: kode_prodi }),
            );
        }
    };
        ...
        <AuthenticatedLayout subtitle="Kelola Program Studi Fakultas Ekonomi">
            ...
            <div className="sibau-prodi-grid">
                {prodis.map((p) => (
                    <div
                        key={p.kode_prodi}
                        className="sibau-card sibau-prodi-card"
                    ...
                    >
                        ...
                        <div>
                            <div
                                className={`sibau-badge ${p.status === 'aktif' ? 'badge-success' : 'badge-danger'}`}
                            ...
                            >
                                {p.daftar_kelas && p.daftar_kelas.length > 0 ? (
                                    p.daftar_kelas.map((cls) => (
                                        <span
                                            key={cls}
                        ...
                        >
                            ...
                            <div style={{ display: 'flex', gap: '4px' }}>
                                ...
                                <button
                                    onClick={() => deleteProdi(p.kode_prodi)}
            ...
            {classModalOpen && selectedProdi && (
                <div className="sibau-modal-overlay">
                    <div className="sibau-modal" style={{ maxWidth: '450px' }}>
                        <div className="sibau-modal-header">
                            <h3 className="sibau-modal-title">
                                🏫 Kelola Kelas - {selectedProdi.nama_prodi}
                            </h3>
                            <button
                        ...
                        <div className="sibau-modal-body">
                            ...
                            >
                                {classForm.data.daftar_kelas.length > 0 ? (
                                    classForm.data.daftar_kelas.map(
                                        (clsName) => (
                                            <div
                                                key={clsName}
                                                style={{
                                    ...
                                    >
                                        Belum ada kelas. Masukkan kode kelas di
                                        atas.
                                    </div>
                                )}
                            </div>
