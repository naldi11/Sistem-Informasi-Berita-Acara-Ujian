...
export default function InputBAU({ schedule, terkunci = false }) {
    ...
    schedule.peserta_ujians.forEach((p) => {
        initialAttendance[p.nim] =
            p.kehadiran === 'belum_ditentukan' ? 'hadir' : p.kehadiran;
        initialNilai[p.nim] = p.nilai !== null ? p.nilai : '';
        initialSignatures[p.nim] = p.tanda_tangan || null;
    });

    const form = useForm({
        // jam_*_aktual boleh null pada draft lama, jadi selalu sediakan cadangan.
        jam_mulai_aktual: (
            bau?.jam_mulai_aktual || schedule.jam_mulai
        ).substring(0, 5),
        jam_selesai_aktual: (
            bau?.jam_selesai_aktual || schedule.jam_selesai
        ).substring(0, 5),
    ...
    const submit = (status) => {
        ...
        Object.keys(form.data.attendance).forEach((nim) => {
            if (
                form.data.attendance[nim] === 'hadir' &&
                status === 'menunggu_validasi' &&
                !form.data.signatures[nim]
            ) {
                missingSigs++;
            }
        });
        ...
        if (status === 'menunggu_validasi' && missingSigs > 0) {
            if (
                !confirm(
                    `Ada ${missingSigs} mahasiswa yang belum tanda tangan. Apakah Anda tetap ingin mengirim berita acara?`,
                )
            ) {
                return;
            }
        ...
        // transform() dipakai agar status_validasi ikut terkirim; menugaskan
        // langsung ke form.data tidak selalu terbaca oleh useForm.
        form.transform((data) => ({ ...data, status_validasi: status })).post(
            route('dosen.berita-acara.save', { jadwal_id: schedule.id }),
        );
    };

    // Calculate totals
    const totalStudents = schedule.peserta_ujians.length;
    const totalHadir = Object.values(form.data.attendance).filter(
        (v) => v === 'hadir',
    ).length;
    const totalAbsen = Object.values(form.data.attendance).filter(
        (v) => v === 'absen',
    ).length;
    ...
    return (
        <AuthenticatedLayout subtitle="Sistem Informasi Berita Acara Ujian">
            ...
            >
                ...
                <div
                    style={{ display: 'flex', gap: '16px', maxWidth: '400px' }}
                >
                    <div style={{ flex: 1 }}>
                        ...
                        <input
                            ...
                            onChange={(e) =>
                                form.setData('jam_mulai_aktual', e.target.value)
                            }
                            required
                        />
                    </div>
            ...
            >
                ...
                >
                    ...
                    <div>
                        HADIR:{' '}
                        <span style={{ color: '#16a34a', fontSize: '11pt' }}>
                            {totalHadir}
                        </span>
                    </div>
                    <div>
                        ABSEN:{' '}
                        <span style={{ color: '#dc2626', fontSize: '11pt' }}>
                            {totalAbsen}
                        </span>
                    </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: '12px' }}>
                    {terkunci ? (
                        <span
                            className="sibau-badge badge-success"
                            style={{ fontSize: '9pt', padding: '8px 12px' }}
                        >
                            🔒 Berita Acara sudah tervalidasi dan tidak dapat
                            diubah lagi.
                        </span>
                    ) : (
                        <>
