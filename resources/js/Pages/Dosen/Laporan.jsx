...
export default function Laporan({ baus }) {
    ...
    // Arsip BAU tervalidasi milik dosen ini jumlahnya kecil, jadi penyaringan
    // cukup dilakukan pada halaman yang sedang ditampilkan.
    const filteredBaus = baus.data.filter((b) =>
                        ...
                        <tbody>
                            {filteredBaus.map((b, idx) => (
                                <tr key={b.id}>
                                    ...
                                    <td>
                                        {b.jam_mulai_aktual.substring(0, 5)} -{' '}
                                        {b.jam_selesai_aktual.substring(0, 5)}
                                    </td>
                                    <td>
                                        {b.jadwal_ujian.ruang} (Kl.{' '}
                                        {b.jadwal_ujian.kelas})
                                    </td>
                                    <td>
                                        Hadir: <strong>{b.jumlah_hadir}</strong>{' '}
                                        / Absen:{' '}
                                        <strong>{b.jumlah_absen}</strong>
                                    </td>
                                    <td>
                                        <a
                                            href={route(
                            ...
                            {filteredBaus.length === 0 && (
                                <tr>
                                    ...
                                    >
                                        Belum ada berita acara ujian yang
                                        divalidasi oleh Kaprodi.
                                    </td>
                                </tr>
                            )}
                        </tbody>
